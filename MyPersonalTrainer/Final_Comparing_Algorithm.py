...
list_of_jsons=['sideview_squat.json']
...
def Extract_Positions_Of_KeyPoints():
    for index in range(len(list_of_jsons)):
        with open(list_of_jsons[index], 'r') as jsonfile:
...
def Number_Of_SquatCycles():
    global Num_Of_Squats,Stage,diff    
    lenOfyList=len(Hip_Y_Positions)
    lenOfMax=len(Max_Y_Hip)
    lenOfMin=len(Min_Y_Hip)
    # print(lenOfMin,lenOfMax)
    for i in range(0,lenOfyList,1):
        backAngle=Back_Angle(i)
        for index in range (min(lenOfMax,lenOfMin)):
            if (Max_Y_Hip[index]==Hip_Y_Positions[i])and(Stage==0) and (170<= (180-backAngle) <=180): 
                Stage=1
                phase1.append(i)
            
            elif (Min_Y_Hip[index]==Hip_Y_Positions[i]) and (Stage==1)and (60<= (90-backAngle)<=90):
                Stage=2
                phase2.append(i)
            
            elif (Max_Y_Hip[index]==Hip_Y_Positions[i])and (Stage==2)and (170<= (180-backAngle) <=180):
                Num_Of_Squats=Num_Of_Squats+1
                up=Hip_Y_Positions[i]
                Stage=1
                phase3.append(i)
    
    x=natsorted(phase1+phase3)
    for i in range (min(len(phase2),len(x))):
        firsthalf=abs(x[i]-phase2[i])
        secondhalf=abs(phase2[i]-x[i+1])    
        diff.append(abs(firsthalf-secondhalf))

def Comparing_Algorithm():
    Direction_Side=Extract_Positions_Of_KeyPoints()
    Number_Of_SquatCycles()
    UpPhases=natsorted(phase1+phase3)
    if Direction_Side=="Left":
        for i in range(len(diff)):
            if 0<= diff[i]<=25:
                # append indecies of keyFrames of each squat cycle
                cycles['keyFramesOf_Cycle'+str(i+1)]=[str(UpPhases[i]),str(phase2[i]),str(UpPhases[i+1])]
                ...
                if ((0.85*Knee_Y_Positions[phase2[i]]) <Hip_Y_Positions[phase2[i]] <(1.15*Knee_Y_Positions[phase2[i]])):
                ...
                if 70<= knee_Angle <=110:
                    ...
                    KneeAngle_Notes.append('Wrong knee angle at cycle'+str(i+1)+'Your knee angle should be between 70 and 110 degree')
                
                # Angle of back at Min position
                
                Hip_Angle=Back_Angle(phase2[i])
                Min_Back_Angle.append(90-Hip_Angle)
                if 60<= (90-Hip_Angle) <= 90:
                ...
                Hip_Angle=Back_Angle(UpPhases[i])
                Max_Back_Angle.append(180-Hip_Angle)
                
                if 170<= (180-Hip_Angle) <= 180:
                    ...
                    MaxBackAngle_Notes.append('Wrong Back angle at cycle'+str(i+1)+'Your Back Angle should be between 70 and 90 degree')
        ...
        for i in range(len(diff)):
            if 0<= diff[i]<=20:
                ...
                if 170<= (180-Hip_Angle) <= 180:
                    ...
                    Max_Back_Angle_Status.append('0')        
                    MaxBackAngle_Notes.append('Wrong Back angle at cycle'+str(i+1)+'Your Back Angle should be between 170 and 180 degree')
    print(Direction_Side)
    return Num_Of_Squats,UpPhases,phase2,Knee_WRT_Toes_25_26,Knee_Notes,Hip_WRT_Knee_23_24,Hip_Notes,Min_Knee_Angle_Status,KneeAngle_Notes,Max_Back_Angle_Status,MaxBackAngle_Notes,Min_Back_Angle_Status,MinBackAngle_Notes            
